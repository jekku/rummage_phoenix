defmodule Rummage.Phoenix.PaginateView do
  ...
  defmacro __using__(opts) do
    quote do
      ...
      def pagination_link(conn, slugs, params, rummage) do
        paginate_params = rummage["paginate"]
        ...
        raw_links = cond do
          ...
          true ->
            [raw_link("Previous", apply(unquote(opts[:helpers]),
              String.to_atom("#{unquote(opts[:struct])}_path"), get_helper_params(conn, slugs, params, per_page, page - 1)))]
        end
        ...
        raw_links = raw_links ++ Enum.map(1..max_page, fn(x) ->
          case page == x do
            ...
            _ -> raw_link(x, apply(unquote(opts[:helpers]),
              String.to_atom("#{unquote(opts[:struct])}_path"), get_helper_params(conn, slugs, params, per_page, x)))
          end
        end)
